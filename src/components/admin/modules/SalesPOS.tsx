...
export const SalesPOS: React.FC = () => {
    ...
    const fetchServices = async () => {
        try {
            ...
            toast.error(t('admin.ui.services.toast.load_error'));
        } finally {
            setLoading(false);
        }
    };
    ...
    const addToCart = (service: Service) => {
        ...
        setShowCart(true);
        toast.success(`${language === 'pt' ? service.name_pt : service.name_en} - ${t('admin.ui.sales.cart.add')}`, {
            icon: '🛍️',
    ...
    const handleCheckout = async () => {
        ...
        try {
            ...
            toast.error(t('admin.ui.sales.toast.sale_error'));
        } finally {
            setProcessing(false);
        }
    };
    ...
    const handleQuickAddClient = async () => {
        const name = prompt(t('admin.ui.clients.prompt_name') || 'Nome da cliente');
        if (!name) return;
        const phone = prompt(t('admin.ui.clients.prompt_phone') || 'Telefone da cliente');
        if (!phone) return;

        try {
            const data = await api.post('/clients', { name, phone });
            setSelectedClient(data);
            toast.success(t('admin.ui.clients.toast.add_success'));
        } catch (error: any) {
            toast.error(t('admin.ui.clients.toast.add_error'));
        }
    };
    ...
    const updateStaffItem = (id: string, staffId: string) => {
        ...
        toast.success(t('admin.ui.sales.toast.staff_assigned') || 'Profissional atribuído!', {
            icon: '👤',
    ...
    const filteredServices = (Array.isArray(services) ? services : []).filter(s => {
        const name = language === 'pt' ? (s.name_pt || '') : (s.name_en || '');
    ...
    if (loading) return (
        <div className="space-y-6 pb-12 animate-in fade-in duration-700 flex flex-col items-center justify-center h-[calc(100vh-200px)] opacity-60">
            ...
            <p className="text-xs font-black uppercase tracking-[0.4em] text-gray-500 animate-pulse">{t('admin.ui.sales.loading')}</p>
        </div>
    );
      ...
      `}</style>
            ...
            <div className={`h-full flex flex-col gap-6 transition-all duration-500 ${showCart ? 'lg:mr-[420px]' : 'w-full'}`}>
                ...
                <div className="px-6 py-6 lg:px-8 bg-white/60 dark:bg-white/5 backdrop-blur-3xl rounded-[2rem] border border-white/50 dark:border-white/5 shadow-xl flex flex-col lg:flex-row lg:items-center justify-between gap-6 transition-all duration-500">
                    <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-3">
                            <h1 className="text-2xl lg:text-3xl font-serif font-black text-gray-900 dark:text-white tracking-tighter uppercase italic">
                                {t('admin.ui.sales.title')}
                            </h1>
                        </div>
                        <p className="text-[9px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.4em] mt-1">{t('admin.ui.sales.description')}</p>
                    </div>
                    ...
                    <div className="flex items-center gap-4 w-full lg:w-auto">
                        <div className="relative flex-1 lg:w-96 group">
                            ...
                            <input
                                type="text"
                                placeholder={t('admin.ui.services.search')}
                                value={searchTerm}
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button
                                ...
                                className={`w-14 h-14 rounded-2xl border transition-all relative flex items-center justify-center shadow-lg ${showCart
                ...
                <div className="flex items-center gap-3 overflow-x-auto pb-4 custom-scrollbar no-scrollbar flex-shrink-0 px-2 lg:px-4">
                    <button
                        ...
                        className={`px-8 py-3.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap border-2 ${!selectedCategory
                    ...
                    {categories.map(cat => (
                        <button
                            ...
                            className={`px-8 py-3.5 rounded-full text-[10px] font-black uppercase tracking-[0.2em] transition-all whitespace-nowrap border-2 ${selectedCategory === cat.id
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-4 2xl:grid-cols-5 gap-6 lg:px-4">
                            {filteredServices.map(service => (
                                <motion.div
                                    ...
                                    className={`group relative bg-white dark:bg-[#1E1E1E] p-6 lg:p-7 rounded-[22px] border transition-all duration-500 flex flex-col justify-between min-h-[170px] cursor-pointer ${lastAddedId === service.id
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex flex-col gap-4">
                                        <div className="flex justify-between items-start">
                                            ...
                                            <div
                                                className={`w-11 h-11 rounded-full flex items-center justify-center transition-all duration-300 shadow-sm active:scale-90 ${lastAddedId === service.id
                                    ...
                                    <div className="relative z-10 flex justify-between items-end mt-6">
                                        <div>
                                            ...
                                            <p className={`text-xl font-black tracking-tighter ${lastAddedId === service.id ? 'text-[#E91E63]' : 'text-[#E91E63] group-hover:text-[#C2185B]'}`}>
                                                {typeof service.price === 'number' ? service.price.toLocaleString() : '0'} <span className="text-[10px] font-sans">MZN</span>
                                            </p>
                                        </div>
                                    </div>
                                </motion.div>
            ...
            >
                ...
                <div className="px-8 pt-8 pb-6 flex items-center justify-between border-b border-pink-50/50 dark:border-white/5 bg-white/80 dark:bg-black/80 backdrop-blur-xl shrink-0">
                    <div>
                        <h2 className="text-2xl font-serif font-black text-gray-900 dark:text-white tracking-tight uppercase">{t('admin.ui.sales.cart.title')}</h2>
                        <p className="text-[10px] font-black text-[#E91E63] uppercase tracking-[0.2em] mt-1">
                            {cart.length} {cart.length === 1 ? t('admin.ui.sales.cart.item_singular') : t('admin.ui.sales.cart.item_plural')}
                        </p>
                    </div>
                    <button
                        onClick={() => {
                            if (window.confirm(`${t('admin.ui.confirm.delete_title')}\n${t('admin.ui.confirm.delete_desc')}`)) {
                                setCart([]);
                            }
                        }}
                ...
                <div className="flex-1 overflow-y-auto px-8 py-6 custom-scrollbar space-y-4 bg-white dark:bg-[#0A0A0A]">
                    {cart.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center opacity-30">
                            ...
                            <p className="text-[11px] font-black text-gray-400 uppercase tracking-widest leading-relaxed">{t('admin.ui.sales.cart.empty')}</p>
                        </div>
                    ) : (
                                    ...
                                    <div className="flex justify-between items-start mb-6">
                                        <div className="max-w-[70%]">
                                            ...
                                            <div className="flex items-center gap-2 mt-2">
                                                ...
                                                >
                                                    <option value="">{t('admin.ui.sales.cart.assign_staff')}</option>
                                                    {staff.map(s => (
                                        ...
                                        <p className="text-sm font-black text-[#E91E63] tracking-tighter">
                                            {(typeof item.price === 'number' ? (item.price * item.quantity) : 0).toLocaleString()} MZN
                                        </p>
                                    </div>
                ...
                <div className="px-8 pt-6 pb-6 border-t border-pink-50/50 dark:border-white/5 bg-[#FAFAFA]/80 dark:bg-black/40 backdrop-blur-md shrink-0">
                    <div className="space-y-3 mb-6">
                        <div className="flex justify-between items-center text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-widest">
                            <span>{t('admin.ui.sales.cart.subtotal') || 'Subtotal'}</span>
                        ...
                        <div className="flex justify-between items-center text-[10px] font-black text-[#E91E63] uppercase tracking-widest cursor-pointer hover:opacity-80 transition-opacity">
                            <span className="flex items-center gap-2"><Tag size={12} /> {t('admin.ui.sales.cart.discount') || 'Adicionar Desconto'}</span>
                            <span>0 MZN</span>
                        </div>
                        <div className="h-px bg-pink-100/50 dark:bg-white/5 my-4" />
                        <div className="flex justify-between items-end">
                            <div>
                                <span className="text-[10px] font-black text-gray-900 dark:text-white uppercase tracking-[0.3em]">{t('admin.ui.sales.cart.total') || 'Total Final'}</span>
                    ...
                    <div className="grid grid-cols-2 gap-3 mb-8">
                        {[
                            { id: 'Dinheiro', label: t('admin.ui.sales.payment.cash') || 'Dinheiro', icon: <Banknote size={18} />, brandColor: 'bg-emerald-500' },
                            { id: 'M-Pesa', label: t('admin.ui.sales.payment.m-pesa') || 'M-Pesa', icon: <div className="w-5 h-5 rounded-full bg-green-600 text-white flex items-center justify-center text-[8px] font-black">M</div>, brandColor: 'bg-green-600' },
                            ...
                            { id: 'Cartão', label: t('admin.ui.sales.payment.card') || 'Cartão', icon: <CreditCard size={18} />, brandColor: 'bg-blue-600' }
                        ].map((method) => (
                            <button
                                key={method.id}
                                onClick={() => setPaymentMethod(method.id)}
                                className={`flex items-center gap-3 p-4 rounded-2xl border-2 transition-all duration-300 active:scale-[0.98] ${paymentMethod === method.id
                            ...
                            >
                                <div className={`w-10 h-10 rounded-xl flex items-center justify-center transition-colors ${paymentMethod === method.id ? `${method.brandColor} text-white` : 'bg-[#FAFAFA] dark:bg-white/5 text-gray-400'
                                    }`}>
                                    {method.icon}
                                </div>
                    ...
                    <button
                        ...
                        className={`group w-full h-[64px] rounded-3xl font-black uppercase tracking-[0.3em] text-[11px] transition-all flex items-center justify-center gap-4 shadow-xl active:scale-[0.98] ${cart.length === 0 || processing
                    ...
                    >
                        {processing ? (
                            ...
                            <>
                                <span>{t('admin.ui.buttons.finish_sale')} — {calculateSubtotal().toLocaleString()} MZN</span>
