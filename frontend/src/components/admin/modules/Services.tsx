...
export const Services: React.FC = () => {
  ...
  const fetchServices = async () => {
    try {
      ...
      toast.error(`${t('admin.ui.services.toast.load_error')}: ${error.message || ''}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingService) {
        ...
        toast.success(t('admin.ui.services.toast.update_success'));
      } else {
      ...
      toast.error(t('admin.ui.feedback.error'));
    }
  };

  const handleDelete = async (id: string) => {
    if (!window.confirm(`${t('admin.ui.confirm.delete_title')}\n${t('admin.ui.confirm.delete_desc')}`)) return;
    
    try {
      await api.delete(`/services/${id}`);
      toast.success(t('admin.ui.services.toast.delete_success'));
      fetchServices();
    } catch (error: any) {
      console.error('Delete Service Error:', error);
      toast.error(`${t('admin.ui.services.toast.delete_error')}: ${error.message}`);
    }
  };

  const filteredServices = (Array.isArray(services) ? services : []).filter(s => {
    const namePt = s.name_pt?.toLowerCase() || '';
    const nameEn = s.name_en?.toLowerCase() || '';
    
    // Support both string category and object category { name_pt, name_en }
    let catStr = '';
    if (typeof s.category === 'string') {
      catStr = s.category.toLowerCase();
    } else if (s.category && typeof s.category === 'object') {
      catStr = (s.category.name_pt || s.category.name_en || '').toLowerCase();
    } else if (s.category_name && typeof s.category_name === 'object') {
       // Fallback for raw backend structure if needed
       catStr = (s.category_name.name_pt || '').toLowerCase();
    }

    return namePt.includes(searchTerm.toLowerCase()) ||
           nameEn.includes(searchTerm.toLowerCase()) ||
           catStr.includes(searchTerm.toLowerCase());
  });
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/40 dark:bg-[#1E1E1E]/40 backdrop-blur-xl p-8 rounded-3xl border border-white/50 dark:border-white/5 shadow-xl shrink-0">
        <div>
          <h1 className="text-3xl font-serif font-black text-gray-800 dark:text-white uppercase tracking-tighter italic">{t('admin.ui.services.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-1 font-medium italic text-sm">{t('admin.ui.services.description')}</p>
          <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-2">{t('admin.ui.services.auxiliary')}</p>
        </div>
        ...
        <div className="flex items-center gap-4">
            <div className="relative group">
                ...
                <input 
                    type="text" 
                    placeholder={t('admin.ui.search.placeholder')}
                    value={searchTerm}
            ...
            >
                <Plus size={16} /> {t('admin.ui.buttons.add_service') || 'Adicionar serviço'}
            </button>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-32 bg-white/20 dark:bg-white/5 backdrop-blur-md rounded-[2rem] border border-dashed border-gray-300 dark:border-white/10">
            ...
            <h3 className="text-2xl font-serif font-bold text-gray-800 dark:text-white mb-2">{t('admin.ui.empty.services')}</h3>
            <p className="text-gray-500 dark:text-gray-400 mb-8">{t('admin.ui.empty.services_helper') || 'Nenhum serviço disponível.'}</p>
            <button 
                onClick={() => handleOpenModal()}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {filteredServices.map((service) => (
                 <div key={service.id} className="bg-white/60 dark:bg-[#1E1E1E]/60 backdrop-blur-xl p-6 rounded-3xl border border-white/50 dark:border-white/5 shadow-sm group hover:shadow-2xl hover:-translate-y-1 active:scale-[0.98] transition-all duration-300 relative overflow-hidden">
                    ...
                    <p className="text-[9px] font-black uppercase tracking-[0.3em] text-pink-500 mb-2">
                        {typeof service.category === 'string' 
                            ? service.category 
                            : (service.category?.name_pt || service.category?.name_en || 'Geral')}
                    </p>
                    ...
                    <div className="flex items-center justify-between pt-5 border-t border-pink-50 dark:border-white/5">
                        ...
                        <div className="px-3 py-1.5 bg-pink-500/5 rounded-xl border border-pink-500/10 group-hover:bg-pink-500 group-hover:text-white transition-all">
                            <span className="text-sm font-serif font-black tracking-tighter">
                                {typeof service.price === 'number' ? service.price.toLocaleString() : '0'} MZN
                            </span>
                        </div>
                    </div>
                </div>
