...
export const Clients: React.FC = () => {
  ...
  const fetchClients = async () => {
    try {
      ...
      toast.error(t('admin.ui.clients.toast.load_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const fetchHistory = async (clientId: string) => {
    try {
      ...
      toast.error(t('admin.ui.clients.toast.history_error'));
    }
  };
  ...
  const handleUpdateClient = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(t('admin.ui.clients.toast.update_error'));
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-white/40 dark:bg-[#1E1E1E]/40 backdrop-blur-xl p-8 rounded-3xl border border-white/50 dark:border-white/5 shadow-xl shrink-0">
        <div>
          <h1 className="text-3xl font-serif font-black text-gray-800 dark:text-white uppercase tracking-tighter italic">{t('admin.ui.clients.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium italic text-sm">{t('admin.ui.clients.description')}</p>
          <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-2">{t('admin.ui.clients.auxiliary')}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
            ...
            >
                <Plus size={16} /> {t('admin.ui.buttons.add_client')}
            </button>
        </div>
      </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/60 dark:bg-white/5 border-b border-pink-50/50 dark:border-white/5 text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">
                <th className="px-10 py-6 italic">{t('admin.ui.clients.table.name')}</th>
                <th className="px-10 py-6 italic">{t('admin.ui.clients.table.contact')}</th>
                <th className="px-10 py-6 italic">{t('admin.ui.clients.table.spent')}</th>
                <th className="px-10 py-6 italic">{t('admin.ui.clients.table.last_visit')}</th>
                <th className="px-10 py-6 text-center italic">{t('admin.ui.clients.table.actions')}</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-pink-50/20 dark:divide-white/5">
              {loading ? (
                <tr>
                  <td colSpan={5} className="px-10 py-20 text-center text-gray-400 font-serif italic">{t('admin.ui.clients.loading')}</td>
                </tr>
                ...
                <tr>
                  <td colSpan={5} className="px-10 py-32 text-center align-middle">
                      <div className="flex flex-col items-center justify-center">
                          ...
                          <h3 className="text-2xl font-serif font-bold text-gray-800 dark:text-white mb-2 italic uppercase tracking-widest">{t('admin.ui.empty.clients')}</h3>
                          <p className="text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">{t('admin.ui.empty.activity')}</p>
                      </div>
                  </td>
                </tr>
