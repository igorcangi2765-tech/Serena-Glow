...
export const Inbox: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'inbox' }, (payload) => {
        ...
        toast.success(`${t('admin.ui.inbox.toast.new_message')}: ${newMessage.name}`, { icon: '📩' });
      })
      .subscribe();
  ...
  const fetchMessages = async () => {
    try {
      ...
      toast.error(t('admin.ui.inbox.toast.load_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const archiveMessage = async (id: string) => {
    try {
      ...
      toast.success(t('admin.ui.feedback.updated'));
    } catch (error) {
      console.error('Error archiving message:', error);
      toast.error(t('admin.ui.inbox.toast.archive_error'));
    }
  };

  const deleteMessage = async (id: string) => {
    if (!confirm(`${t('admin.ui.confirm.delete_title')}\n${t('admin.ui.confirm.delete_desc')}`)) return;
    ...
    try {
      ...
      toast.success(t('admin.ui.feedback.removed'));
    } catch (error) {
      console.error('Error deleting message:', error);
      toast.error(t('admin.ui.inbox.toast.delete_error'));
    }
  };
  ...
  const handleReply = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t('admin.ui.inbox.toast.reply_success'), { icon: '📨' });
      setIsReplyOpen(false);
      setReplyText('');
    } catch (error) {
      toast.error(t('admin.ui.inbox.toast.reply_error'));
    }
  };
  ...
  return (
    <div className="h-[calc(100vh-180px)] flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-4xl font-serif font-bold text-gray-800 dark:text-white uppercase tracking-tighter italic">{t('admin.ui.inbox.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-2 font-medium italic">{t('admin.ui.inbox.description')}</p>
          <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-2">{t('admin.ui.inbox.auxiliary')}</p>
        </div>
        ...
        <div className="flex items-center gap-2">
            ...
            >
                <option value="all">{t('admin.ui.inbox.filters.all')}</option>
                <option value="unread">{t('admin.ui.inbox.filters.unread')}</option>
                <option value="read">{t('admin.ui.inbox.filters.read')}</option>
                <option value="replied">{t('admin.ui.inbox.filters.replied')}</option>
                <option value="archived">{t('admin.ui.inbox.filters.archived')}</option>
            </select>
        </div>
      </div>
          ...
          <div className="flex-grow overflow-y-auto custom-scrollbar p-4 space-y-4">
            {loading ? (
              <div className="flex flex-col items-center justify-center h-full opacity-30">
                ...
                <p className="text-xs font-black uppercase tracking-widest">{t('admin.ui.inbox.loading')}</p>
              </div>
              ...
              <div className="flex flex-col items-center justify-center h-full opacity-30 space-y-4 px-10 text-center">
                ...
                <p className="text-xs font-black uppercase tracking-widest leading-relaxed">{t('admin.ui.empty.activity')}</p>
              </div>
             ...
             {selectedMessage ? (
               ...
               <div className="flex flex-col items-center justify-center h-full opacity-30 p-20 text-center">
                 <div className="w-32 h-32 bg-pink-100 dark:bg-pink-900/10 rounded-full flex items-center justify-center mb-8 shadow-inner">
                  ...
                  <h3 className="text-2xl font-serif text-gray-500 dark:text-gray-400 mb-4 tracking-tighter italic font-medium">{t('admin.selectMessage')}</h3>
                  <div className="text-[10px] font-black uppercase tracking-[0.2em] max-w-sm leading-relaxed text-gray-400 text-center mx-auto">
                    {t('admin.ui.emptyStates.notifications')}
                  </div>
               </div>
             )}
