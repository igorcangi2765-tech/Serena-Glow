...
export const Billing: React.FC = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      toast.error(t('admin.ui.billing.toast.load_error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const generatePDF = (doc: Document) => {
    ...
    pdf.text(t('admin.ui.billing.pdf.company'), 105, 25, { align: 'center' });
    
    pdf.setFontSize(10);
    pdf.setTextColor(150);
    pdf.text(t('admin.ui.billing.pdf.subtitle'), 105, 32, { align: 'center' });
    pdf.text(t('admin.ui.billing.pdf.address'), 105, 37, { align: 'center' });
    ...
    pdf.setFontSize(10);
    pdf.setTextColor(80);
    pdf.text(`${t('admin.ui.billing.table.date').toUpperCase()}: ${format(new Date(doc.created_at), 'dd/MM/yyyy HH:mm')}`, 190, 60, { align: 'right' });
    pdf.setTextColor(31, 41, 55);
    pdf.text(`CLIENTE: ${doc.sales?.clients?.name || t('admin.ui.sales.walk_in') || 'Venda de Balcão'}`, 20, 80);
    
    // Main Table
    autoTable(pdf, {
      startY: 90,
 head: [[t('admin.ui.billing.pdf.description'), t('admin.ui.billing.pdf.qty'), t('admin.ui.billing.pdf.unit'), t('admin.ui.billing.pdf.total')]],
      body: [
    ...
    pdf.setFontSize(8);
    pdf.text(t('admin.ui.billing.pdf.client_signature') || 'ASSINATURA DO CLIENTE', 50, finalY + 5, { align: 'center' });
    pdf.text(t('admin.ui.billing.pdf.company_signature') || 'ASSINATURA SERENA GLOW', 160, finalY + 5, { align: 'center' });
    
    // Footer Legal
    pdf.setFontSize(8);
    pdf.setTextColor(180);
    pdf.text(t('admin.ui.billing.pdf.footer_legal') || 'Obrigada pela sua preferência. Este documento serve como comprovativo de faturação oficial.', 105, 285, { align: 'center' });

    pdf.save(`${doc.doc_number}.pdf`);
    toast.success(t('admin.ui.billing.toast.download_success') || 'Documento descarregado com sucesso!', { 
      ...
      >
        ...
        <div className="p-10 max-h-[70vh] overflow-y-auto no-scrollbar">
           <div className="text-center mb-10">
              <h2 className="text-3xl font-serif font-black text-pink-600 tracking-tighter">{t('admin.ui.billing.pdf.company')}</h2>
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em]">{t('admin.ui.billing.pdf.subtitle')}</p>
           </div>

           <div className="grid grid-cols-2 gap-10 mb-10 text-xs">
              <div className="space-y-2">
                 <p className="font-black text-gray-400 uppercase tracking-widest">{t('admin.ui.billing.pdf.issued_to') || 'Emitido para:'}</p>
                 <p className="text-lg font-serif font-black text-gray-900 dark:text-white">{doc.sales?.clients?.name || t('admin.ui.sales.walk_in') || 'Venda de Balcão'}</p>
                 <p className="text-gray-500">{doc.sales?.clients?.phone || t('admin.ui.clients.no_phone') || 'Telefone não registado'}</p>
              </div>
              <div className="space-y-2 text-right">
                 <p className="font-black text-gray-400 uppercase tracking-widest">{t('admin.ui.billing.table.details') || 'Detalhes:'}</p>
                 <p className="font-bold text-gray-800 dark:text-white">{doc.type === 'invoice' ? t('admin.ui.billing.invoice') : t('admin.ui.billing.receipt')} Profissional</p>
           ...
           <table className="w-full text-left mb-10">
              <thead>
                 <tr className="border-b-2 border-pink-100 dark:border-white/10 text-[10px] font-black text-gray-400 uppercase tracking-widest">
                    <th className="py-4">{t('admin.ui.billing.pdf.description')}</th>
                    <th className="py-4 text-center">{t('admin.ui.billing.pdf.qty')}</th>
                    <th className="py-4 text-right">{t('admin.ui.billing.table.total')}</th>
                 </tr>
              </thead>
              <tbody className="divide-y divide-pink-50/10 dark:divide-white/5">
                 {doc.sales?.sale_items?.map((item, i) => (
                    ...
                    <tr className="text-sm font-bold text-gray-700 dark:text-gray-300">
                       <td className="py-4">{t('admin.ui.billing.pdf.fallback_desc') || 'Serviços Estéticos Diversos'}</td>
           ...
           <div className="bg-gray-50 dark:bg-zinc-950 p-6 rounded-2xl border-2 border-pink-100 dark:border-white/5 flex justify-between items-center">
              <span className="font-black text-gray-900 dark:text-white uppercase tracking-widest">{t('admin.ui.sales.cart.total') || 'Total Final'}</span>
        ...
        <div className="p-8 bg-gray-50 dark:bg-zinc-950 border-t border-pink-100 dark:border-white/5 flex gap-4">
           ...
           >
             <Download size={18} /> {t('admin.ui.buttons.download_pdf') || 'Descarregar PDF'}
           </button>
           <button 
             onClick={onClose}
           ...
           >
             {t('admin.ui.buttons.close') || 'Fechar'}
           </button>
        </div>
      </motion.div>
    </motion.div>
  ...
  return (
    <div className="space-y-6 pb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-10 pb-2">
        <div>
          <h1 className="text-5xl font-serif font-black text-gray-900 dark:text-white uppercase tracking-tighter leading-none italic">{t('admin.ui.billing.title')}</h1>
          <p className="text-gray-500 dark:text-gray-400 mt-4 font-medium italic text-base">{t('admin.ui.billing.description')}</p>
          <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-2">{t('admin.ui.billing.auxiliary')}</p>
        </div>
        ...
        <div className="flex gap-4">
            ...
            >
             ...
             <button 
                onClick={() => toast.success(t('admin.ui.billing.toast.auto_billing') || 'Faturação automática activa!', { icon: '🤖' })}
              ...
              >
                <Plus size={16} /> {t('admin.ui.billing.new') || 'Novo Documento'}
              </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
              { title: t('admin.ui.billing.stats.monthly'), value: `${stats.totalMonthly.toLocaleString()} MZN`, icon: Receipt, color: 'emerald', trend: `${stats.growth >= 0 ? '+' : ''}${stats.growth.toFixed(1)}%` },
              { title: t('admin.ui.billing.stats.docs'), value: stats.docCount, icon: FileSignature, color: 'pink', trend: 'Total' },
              { title: t('admin.ui.billing.stats.pending'), value: `${stats.pendingDebt.toLocaleString()} MZN`, icon: CreditCard, color: 'rose', trend: 'Atraso' },
          ].map((stat, idx) => (
              <motion.div 
                key={idx}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-white/60 dark:bg-white/5 border-b-2 border-pink-100 dark:border-white/10 text-[11px] font-black text-gray-400 uppercase tracking-[0.25em]">
                <th className="px-6 py-8">{t('admin.ui.billing.table.number')}</th>
                <th className="px-6 py-8">{t('admin.ui.billing.table.type')}</th>
                <th className="px-6 py-8">{t('admin.ui.billing.table.client')}</th>
                <th className="px-6 py-8">{t('admin.ui.billing.table.date')}</th>
                <th className="px-6 py-8 text-right font-black text-gray-900 dark:text-white tracking-widest uppercase">{t('admin.ui.billing.table.total')}</th>
                <th className="px-6 py-8 text-center">{t('admin.ui.billing.table.actions')}</th>
              </tr>
            </thead>
              ...
              {loading ? (
                <tr>
                  <td colSpan={6} className="px-10 py-20 text-center">
                      ...
                      <p className="text-[10px] font-black uppercase tracking-widest text-gray-400">{t('admin.ui.billing.loading') || 'Consultando arquivos...'}</p>
                  </td>
                </tr>
              ) : documents.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-10 py-32 text-center align-middle">
                      <div className="flex flex-col items-center justify-center">
                          ...
                          <h3 className="text-2xl font-serif font-bold text-gray-800 dark:text-white mb-2 italic uppercase tracking-widest">{t('admin.ui.billing.empty') || 'Sem documentos'}</h3>
                          <p className="text-gray-500 dark:text-gray-400 text-[10px] font-black uppercase tracking-[0.2em]">{t('admin.ui.billing.auxiliary')}</p>
                      </div>
                  </td>
                </tr>
              ) : documents.map((doc) => (
                <tr key={doc.id} className="hover:bg-white/40 dark:hover:bg-white/5 transition-all group border-b border-pink-50/10 dark:border-white/5 last:border-0">
                  ...
                  <td className="px-6 py-8">
                    <span className={`px-5 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest whitespace-normal ${
                      ...
                      {doc.type === 'invoice' ? t('admin.ui.billing.invoice') : t('admin.ui.billing.receipt')}
                    </span>
                  </td>
                  <td className="px-6 py-8">
                    <div className="flex items-center gap-4">
                        ...
                        <span className="text-sm font-black text-gray-700 dark:text-gray-200 whitespace-normal">{doc.sales?.clients?.name || t('admin.ui.sales.walk_in') || 'Venda de Balcão'}</span>
                    </div>
                  </td>
                  <td className="px-6 py-10">
                  ...
                  <td className="px-6 py-10 text-center">
                    <div className="flex justify-center gap-3">
                      <button 
                        ...
                        title={t('admin.ui.buttons.preview') || 'Visualizar Recibo'}
                      >
                        <Eye size={18} />
                      </button>
                      <button 
                        ...
                        title={t('admin.ui.buttons.download_pdf') || 'Descarregar PDF'}
                      >
                        <Download size={18} />
                      </button>
                      <button 
                        ...
                        title={t('admin.ui.buttons.send_email') || 'Enviar por Email'}
                      >
                        <Send size={16} />
                      </button>
                    </div>
      ...
      <div className="bg-gradient-to-br from-pink-500 to-rose-600 p-8 md:p-10 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-12">
              <div className="flex-1 text-left space-y-4">
                  <h3 className="text-3xl md:text-4xl font-serif font-black uppercase tracking-tighter leading-none">{t('admin.ui.billing.banner.title') || 'Relatórios Financeiros Avançados'}</h3>
                  <p className="text-base md:text-lg opacity-90 leading-relaxed italic font-medium max-w-2xl">
                      "{t('admin.ui.billing.banner.description') || 'Utilize os dados de facturação para entender os picos de procura no seu Studio.'}"
                  </p>
              </div>
