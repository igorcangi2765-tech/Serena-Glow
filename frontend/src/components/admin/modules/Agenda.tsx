...
export const Agenda: React.FC = () => {
  ...
  const fetchAppointments = async () => {
    try {
      ...
      toast.error(`${t('admin.ui.agenda.toast.load_error')}: ${error?.message || ''}`);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="flex flex-col h-[calc(100vh-180px)] gap-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white/40 dark:bg-[#1E1E1E]/40 backdrop-blur-xl p-8 rounded-3xl border border-white/50 dark:border-white/5 shadow-xl shrink-0 mb-8">
        <div>
          <h1 className="text-3xl font-serif font-black text-gray-800 dark:text-white uppercase tracking-tighter italic">{t('admin.ui.agenda.title')}</h1>
          ...
          <p className="text-[10px] font-black text-pink-500 uppercase tracking-widest mt-2">{t('admin.ui.agenda.auxiliary')}</p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-4">
             ...
             <button 
              ...
              >
                <Plus size={16} /> {t('admin.ui.buttons.create_booking')}
              </button>
        </div>
      </div>
        ...
        {loading && (
            <div className="absolute inset-0 bg-white/20 backdrop-blur-sm z-[100] flex items-center justify-center font-serif italic text-gray-500">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest opacity-40">{t('admin.ui.agenda.loading')}</span>
                </div>
            </div>
        )}
        ...
        <form onSubmit={handleCreateBooking} className="p-2 space-y-6">
            ...
            <div className="space-y-2">
                ...
                >
                    ...
                    {services.map(s => <option key={s.id} value={s.id}>{s.name_pt} - {typeof s.price === 'number' ? s.price.toLocaleString() : '0'} MZN</option>)}
                </select>
            </div>
