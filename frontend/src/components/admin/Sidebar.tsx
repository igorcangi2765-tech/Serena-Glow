...
export const Sidebar: React.FC<SidebarProps> = ({ activeTab, setActiveTab, onLogout, isOpen = true, onClose, isCollapsed = false, onToggleCollapse }) => {
  ...
  const menuItems = [
    { id: 'dashboard', icon: <LayoutDashboard size={20} />, label: t('admin.sidebar.dashboard') },
    { id: 'agenda', icon: <Calendar size={20} />, label: t('admin.sidebar.agenda') },
    { id: 'sales', icon: <ShoppingBag size={20} />, label: t('admin.sidebar.sales') },
    { id: 'billing', icon: <FileText size={20} />, label: t('admin.sidebar.reports') },
    { id: 'clients', icon: <Users size={20} />, label: t('admin.sidebar.clients') },
    { id: 'services', icon: <Scissors size={20} />, label: t('admin.sidebar.services') },
    { id: 'marketing', icon: <Megaphone size={20} />, label: t('admin.sidebar.marketing') },
    { id: 'gallery', icon: <ImageIcon size={20} />, label: t('admin.sidebar.gallery') },
    { id: 'settings', icon: <SettingsIcon size={20} />, label: t('admin.sidebar.team') },
  ];
        ...
        <div className={`flex items-center gap-4 mb-8 p-4 bg-gradient-to-br from-pink-50 to-rose-50 dark:from-pink-900/10 dark:to-rose-900/10 rounded-[1.5rem] border border-pink-100/30 dark:border-pink-500/10 shadow-sm group ${isCollapsed ? 'justify-center p-2' : ''}`}>
          ...
          {!isCollapsed && (
            <div className="overflow-hidden flex flex-col justify-center">
                <p className="text-sm font-bold text-gray-800 dark:text-[#EAEAEA] truncate leading-tight">{t('admin.sidebar.brand')}</p>
                <p className="text-xs text-pink-600 dark:text-pink-400 font-bold uppercase tracking-widest opacity-80 mt-1 truncate">{t('admin.sidebar.tagline')}</p>
            </div>
          )}
        </div>
